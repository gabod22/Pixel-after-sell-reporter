...
def write_range(data, wks: Worksheet, row, col: str):
    wks.update("{}{}".format(col, row), data)
    # insert_notes(notes, row, wks)


def write_in_last_row(data:list, wks: Worksheet):
    values_list = wks.col_values(1)
    values_len = len(values_list)-1
    data[0].insert(0,values_len)
    wks.update("{}{}".format("A", values_len+2), data)
...
def get_spreadsheet(json_file, doc_id, tab_id):
    ...
    * tab_id
    ...
    try:
        worksheet = gspread_client.open_by_key(doc_id).worksheet(tab_id)
