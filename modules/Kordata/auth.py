        ...
        if "idUsuario" in data_response:
            ...
            if len(data_response["bitacoraAccesoDto"]["secciones"]) > 0:
                return {
                    "error": {
                        "type": "already_logged",
                        "message": error_messages["already_logged"],
                        "data": data_response,
                    }
                }
            ...
            return {
                "error": {
                    "type": "pass_email_wrong",
                    "message": data_response["errorMessage"],
                }
            }
        else:
            return {
                "error": {"type": "unknown", "message": data_response["errorMessage"]}
            }
...
def check_valid_session():
    ...
    if get_current_token():
        ...
        headers = {
            "user-agent": "pixel-reporter/0.0.1",
